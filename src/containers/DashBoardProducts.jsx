        ...
        <table id="table-id" className={s.tabla}>
          ...
          {productOff.map((o) =>
            <tbody key={o.id}>
              <tr>
                <td>{o.id}</td>
                <td><img className={s.productImg} src={o.img === null ? adminPic : o.img}/></td>
                <td>{o.title.substr(0, 30)+"..."}</td>
                <td>{o.stock}</td>
                <td>{o.category.name}</td>
                <td>${o.price}</td>
                <td>{o.status === "active" ? (<div className={s.statusCont} ><div className={s.statusProdOk}></div><div>{o.status}</div></div>) : (<div className={s.statusCont} ><div className={s.statusProdNo}></div><div>{o.status}</div></div>)}</td>              
                <td><Link to={`/update/${o.id}`}><button className={s.buttonEdit}>Edit Product</button></Link></td>
              </tr>
            </tbody>
          )}
        </table>
